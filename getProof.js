...
module.exports = class GetProof{
  ...
  async receiptProof(txHash){
    ...
    await Promise.all(receipts.map((siblingReceipt, index) => {
      ...
      // console.log(siblingReceipt.type,111, Buffer.concat([Buffer.from(siblingReceipt.type),serializedReceipt]))
      return promisfy(tree.put, tree)(siblingPath,  Buffer.concat([toBuffer(siblingReceipt.type),serializedReceipt]))
    }))
